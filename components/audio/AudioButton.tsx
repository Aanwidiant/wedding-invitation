    ...
    return (
        <div
            className={`fixed right-6 bottom-6 z-40 transition-all duration-500 ${
                isOpened ? 'scale-100 opacity-100' : 'pointer-events-none scale-90 opacity-0'
            }`}
        >
            {isPlaying && (
                <span className='bg-primary/30 pointer-events-none absolute -inset-1.5 animate-pulse rounded-full' />
            )}
